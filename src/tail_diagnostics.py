...
def marginal_tail_check(df: pd.DataFrame, dist) -> pd.DataFrame:
    ...
    out["boundary_shortfall"] = (
        out["emp_p_boundary"] - out["model_p_boundary"]
    )  # >0 => model too thin
...
def main() -> int:
    ...
    # headline signals
    corr_gap_rrr = (
        float(np.corrcoef(np.arange(len(gap)), gap["gap"].to_numpy())[0, 1])
        if len(gap) > 2
        else float("nan")
    )
    ...
    L += [
        f"Train split, {len(train):,} balls. WP = era-adjusted RRR Markov (the surface "
    ...
    L += [
        ...
        f"it.",
        "",
        "This makes the diagnosis airtight. If the one-ball marginals are correct AND "
        "balls were conditionally independent given (b,w,r), the backward-induction "
        "WP would equal E[y|b,w,r] exactly. It does not (Diagnostic 1). With the "
        "marginals ruled out, the only remaining cause is conditional DEPENDENCE -- "
        "ball-to-ball correlation given the state.",
        "",
    ]
    ...
    L += [
        ...
        "",
        "Same Diagnostic-1 table on the held-out seasons. CAVEAT: unlike the "
        "train-on-train tables above, the era shift IS in frame here (the model is "
        "fit on lower-scoring seasons), so the LEVEL is expected to move toward "
        "under-prediction; the robustness check is whether the non-monotone "
        "sign-flip STRUCTURE persists, not the level.",
        "",
        ...
        + (
            ...
            if flips_oos
            else "The positive band does not survive out of sample -- consistent with "
    ...
    print(
        f"OOS sign flip: {flips_oos}  worst OOS gap: RRR {worst_oos.name} {worst_oos['gap']:+.4f}"
    )
    print(
        f"high-RRR boundary shortfall (emp - model): {tail_hi['boundary_shortfall']:+.4f} "
